 ...
 *
 */
package com.kdgdev.javaedit.editor.codeedit;
...
public class JAsmParser {
    ...
    public InstructionList parse(String code, ConstantPoolGen cpg)
        ...
        for (String codeLine : codeLines) {
            ...
            if (beginsWithWhitespace(fullInstr) && switchMode == 1) {
                ...
                if (instrName.equals("bipush")) {
                    ...
                    //IINC ist = new IINC(arg1, arg2);
                    //Was going to write something here to detect if the previous inst was
                    // WIDE but that's useless
                    //if(instructions.getEnd().getInstruction() instanceof
        ...
        for (TempSwitchData lookupSwitche : lookupSwitches) {

            int targetArrSize = 0;
            for (int j = 0; j < lookupSwitche.getBranchPairs().size(); j++) {
                BranchPair bp = lookupSwitche.getBranchPairs().get(j);
                if (bp.source != -1) {
                    targetArrSize++;
                }
            }
            ...
            for (int j = 0; j < lookupSwitche.getBranchPairs().size(); j++) {
                ...
                if (bp.source != -1) {
                    ...
                    targetInstrs[count] = instructionHandleList
                            .get(bp.target - 1);
                    count++;
                } else {
                    defaultTarget = instructionHandleList
                            .get(bp.target - 1);
                }
            }

            LOOKUPSWITCH lus = (LOOKUPSWITCH) lookupSwitche.ih.getInstruction();
            lus.setMatchesTargets(targets, targetInstrs);
            lus.setTarget(defaultTarget);

        }

        for (TempSwitchData tsd : tableSwitches) {
            int targetArrSize = 0;
            for (int j = 0; j < tsd.getBranchPairs().size(); j++) {
                BranchPair bp = tsd.getBranchPairs().get(j);
                if (bp.source != -1) {
                    targetArrSize++;
                }
            }
    ...
    private boolean isDefaultLine(String arg) {
        ...
        return args.length == 2 & args[0].trim().equals("default");
    }
    ...
    private int countLines(String[] codeLines) {
        int count = 0;
        for (String codeLine : codeLines) {
            if (!beginsWithWhitespace(codeLine)) {
                count++;
            }
        }
        return count;
    ...
    private short getSingleShortArg(String[] instrElems, int line) {
        try {
            return Short.parseShort(instrElems[1]);
        } catch (ArrayIndexOutOfBoundsException exc1) {
            parseException.addError(JAsmParseException.MISSING_ARGUMENTS,
                    instrElems[0], line);
            return 0;
    ...
    private int getSingleIntArg(String[] instrElems, int line) {
        try {
            return Integer.parseInt(instrElems[1]);
        } catch (ArrayIndexOutOfBoundsException exc1) {
            parseException.addError(JAsmParseException.MISSING_ARGUMENTS,
                    instrElems[0], line);
            return 0;
    ...
    private byte getSingleByteArg(String[] instrElems, int line) {
        try {
            return Byte.parseByte(instrElems[1]);
        } catch (ArrayIndexOutOfBoundsException exc1) {
            parseException.addError(JAsmParseException.MISSING_ARGUMENTS,
                    instrElems[0], line);
            return 0;
    ...
    private int getConstRef4ldc(String[] instrElems, ConstantPoolGen cpg, int line) {
        if (instrElems.length < 2) {
            parseException.addError(JAsmParseException.MISSING_ARGUMENTS, instrElems[0], line);
            return 0;
        }
        ...
        if (instrElems[1].startsWith("\"")) {
            StringBuilder sb = new StringBuilder(instrElems[1]);
        ...
        if(cpg.lookupClass(instrElems[1]) != -1) {
            return cpg.lookupClass(instrElems[1]);
        }

        parseException.addError(JAsmParseException.ARG_TYPE_ERROR, instrElems[0], line);
    ...
    public String getClassFromFullMethod(String fullMethod) {
        if (!fullMethod.contains("(")) {
            return getClassFromFieldName(fullMethod);
        }
        String classAndMeth = fullMethod.substring(0, fullMethod.indexOf('('));
        return getClassFromFieldName(classAndMeth);
    }

    private String[] getMethodArguments(String[] instrElems, int line) {
        if (instrElems.length < 2) {
            parseException.addError(JAsmParseException.MISSING_ARGUMENTS,
                    instrElems[0], line);
            return null;
    ...
    public String getMethodFromFullMethod(String fullMethod) {
        if (!fullMethod.contains("(")) {
            return getFieldFromFieldName(fullMethod);
        }
        String classAndMeth = fullMethod.substring(0, fullMethod.indexOf('('));
        return getFieldFromFieldName(classAndMeth);
    }
    ...
    public String getDescrFromFullMethod(String fullMethod) {
        ...
        return fullMethod.substring(fullMethod.indexOf('('),
                fullMethod.length());
    }
